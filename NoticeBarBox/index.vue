  ...
  <div class="notice-bar-box">
    <div class="notice-bar-content" :style="noticeBarStyle">
      ...
      <div v-if="config.icon" class="notice-icon">
        <img :src="getFullImageUrl(config.icon)" :alt="config.alt || '公告图标'" draggable="false" />
      </div>

      <!-- 文字内容区域 -->
      <div class="notice-text-container">
        <div class="notice-text" :style="textStyle" ref="textRef">
...
const config = computed(() => {
  ...
  return {
    noticeText: configParamJson.noticeText || '请填写内容，如果过长，将会在手机上滚动显示',
    icon: configParamJson.icon || '',
    alt: configParamJson.alt || '公告图标',
    backColor: configParamJson.backColor || 'rgb(255, 248, 233)',
    textColor: configParamJson.textColor || 'rgba(100, 101, 102)',
    closable: configParamJson.closable ?? false,
    scrollable: configParamJson.scrollable ?? true
  }
})
